...
import { FaSearch, FaMapMarkerAlt, FaStar, FaFilter, FaArrowRight, FaMotorcycle, FaCheckCircle } from "react-icons/fa";
...
  // --- API: Fetch Location Data ---
  useEffect(() => {
    const fetchLocations = async () => {
      try {
        const res = await axios.get(`http://localhost:5000/api/restaurant-locations`);
...
    return apiRestaurants.filter((res) => {
      const name = res.r_name ? res.r_name.toLowerCase() : "";
      const address = res.address ? res.address.toLowerCase() : "";
      
      const matchesSearch = name.includes(searchTerm.toLowerCase());
      const matchesArea = selectedArea === "All" || address === selectedArea.toLowerCase();
      
...
                <RestaurantCard key={item.id} item={item} idx={idx} navigate={navigate} />
              ))
            ) : (
              <div className="col-span-full text-center py-20">
                <p className="text-slate-400 font-bold text-lg">No restaurants found in this area!</p>
              </div>
            )}
          </AnimatePresence>
        </div>
...
            <p className="text-[11px] font-bold truncate uppercase tracking-tight">{item.address || "Sylhet"}</p>
          </div>
        </div>
      </div>
